...
class QuantumEigenvalueDemo:
    ...
    def create_ybus_matrix(self):
        ...
        self.ybus = sparse_random(self.n_buses, self.n_buses, density=density, 
                                  format='dense', random_state=42)
    ...
    def setup_visualization(self):
        ...
        self.fig.suptitle('QuantumGridOS Innovation #2: Quantum Eigenvalue Algorithm (QPSEA)', 
    ...
    def show_ybus_sparsity(self):
        ...
        self.ax1.clear()
        self.ax1.set_title(f'Y-bus Matrix ({self.n_buses}×{self.n_buses})', 
        ...
        self.ax1.text(0.5, -0.1, f'Sparsity: {sparsity:.1f}%', 
                     transform=self.ax1.transAxes, ha='center', fontsize=12, 
                     color='#ffd43b', fontweight='bold')
        
        self.ax1.text(0.5, -0.15, 'Key insight: Each bus connects to only 2-4 others', 
    ...
    def show_complexity_comparison(self):
        ...
        # Plot
        self.ax2.semilogy(sizes, classical_ops, 'o-', color='#ff6b6b', 
                         linewidth=3, markersize=8, label='Classical O(n³)')
        self.ax2.semilogy(sizes, quantum_ops, 's-', color='#00ff88', 
                         linewidth=3, markersize=8, label='Quantum O(log n)')
        ...
        self.ax2.text(100, quantum_ops[3]*10, f'{speedup_100:.0f}× faster', 
                     fontsize=11, color='#ffd43b', fontweight='bold')
    ...
    def show_eigenvalue_spectrum(self):
        ...
        scatter = self.ax3.scatter(real_parts, imag_parts, c=criticality, 
                                  cmap='coolwarm', s=50, edgecolors='white', 
                                  linewidth=1, alpha=0.8)
        
        # Highlight critical eigenvalue
        crit_idx = np.argmin(criticality)
        self.ax3.scatter(real_parts[crit_idx], imag_parts[crit_idx], 
                        s=200, color='#ffd43b', marker='*', 
    ...
    def show_quantum_circuit(self):
        ...
        for i in range(n_ancilla):
            y = n_ancilla - i - 1
            rect = Rectangle((0.5, y-0.15), 0.3, 0.3, 
                           facecolor='#339af0', edgecolor='white')
            self.ax4.add_patch(rect)
            self.ax4.text(0.65, y, 'H', fontsize=7, ha='center')
        
        # Sparse evolution (our innovation)
        rect = Rectangle((2, 0), 2, n_ancilla + n_system, 
                        facecolor='#00ff88', alpha=0.3, edgecolor='#00ff88', linewidth=2)
        self.ax4.add_patch(rect)
        self.ax4.text(3, (n_ancilla + n_system)/2, 'Sparse\nY-bus\nEvolution', 
                     fontsize=10, ha='center', fontweight='bold')
        
        # QFT inverse
        rect = Rectangle((4.5, 0), 1, n_ancilla, 
                        facecolor='#f59f00', alpha=0.5, edgecolor='white')
        self.ax4.add_patch(rect)
        ...
        self.ax4.text(0.5, -0.08, '🔬 Innovation: Sparse evolution exploits grid topology', 
                     transform=self.ax4.transAxes, ha='center', fontsize=11, 
                     color='#00ff88', style='italic')
    ...
    def animate_performance(self):
        ...
        for i, prog in enumerate(classical_progress[:20]):  # Show first 200 iterations
            rect = Rectangle((i*0.4, 1), 0.35, 0.8, 
                           facecolor='#ff6b6b', alpha=0.6)
            self.ax5.add_patch(rect)
        
        # Quantum bar
        for i, prog in enumerate(quantum_progress):
            rect = Rectangle((i*0.8, 0), 0.7, 0.8, 
        ...
        self.ax5.text(0.5, 2.2, f'Classical: {total_classical:,} ms', 
                     fontsize=11, color='#ff6b6b')
        self.ax5.text(0.5, -0.3, f'Quantum: {total_quantum} ms', 
                     fontsize=11, color='#00ff88')
        
        # Winner
        self.ax5.text(8, 1, f'{total_classical/total_quantum:.0f}× faster!', 
                     fontsize=14, color='#ffd43b', fontweight='bold')
    ...
    def show_speedup_scaling(self):
        ...
        for size, speedup_val in zip(marker_sizes, marker_speedups):
            self.ax6.scatter(size, speedup_val, s=100, color='#ffd43b', 
                           edgecolor='white', linewidth=2, zorder=5)
            self.ax6.text(size, speedup_val*1.5, f'{speedup_val:.0f}×', 
        ...
        self.ax6.text(0.5, 0.9, '📈 Exponential advantage as grid grows', 
                     transform=self.ax6.transAxes, ha='center', fontsize=12, 
                     color='#ffd43b', fontweight='bold')

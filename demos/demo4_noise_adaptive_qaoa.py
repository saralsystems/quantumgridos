...
class NoiseAdaptiveQAOADemo:
    ...
    def setup_visualization(self):
        ...
        self.fig.suptitle(
            "QuantumGridOS Innovation #4: Noise-Adaptive QAOA",
            fontsize=20,
            fontweight="bold",
            color="#00ff88",
        )
    ...
    def show_renewable_uncertainty(self):
        ...
        self.ax1.fill_between(
            self.t, 0, self.solar_actual, alpha=0.6, color="#ffd43b", label="Solar"
        )
        self.ax1.fill_between(
            self.t,
            self.solar_actual,
            self.renewable_total,
            alpha=0.6,
            color="#339af0",
            label="Wind",
        )
        ...
        # Plot demand
        self.ax1.plot(self.t, self.load_demand, "--", color="#ff6b6b", linewidth=2, label="Demand")
        ...
        self.ax1.text(
            0.5,
            0.9,
            f"Average Uncertainty: ±{avg_uncertainty:.1f}%",
    ...
    def show_quantum_noise_mapping(self):
        ...
        for i, (unc, noise, color, y) in enumerate(
            zip(uncertainties, noise_types, colors, y_positions)
        ):
            # Source (uncertainty)
            circle1 = Circle(
                (0.2, y), 0.08, facecolor=color, alpha=0.7, edgecolor="white", linewidth=2
            )
            ...
            circle2 = Circle(
                (0.8, y), 0.08, facecolor="#00ff88", alpha=0.7, edgecolor="white", linewidth=2
            )
        ...
        self.ax2.text(
            0.5,
            0.05,
            "🔬 Innovation: Quantum noise models real-world uncertainty",
    ...
    def show_solution_robustness(self):
        ...
        # Violin plot
        parts1 = self.ax3.violinplot(
            [traditional_outcomes], positions=[1], widths=0.7, showmeans=True
        )
        parts2 = self.ax3.violinplot([nag_outcomes], positions=[2], widths=0.7, showmeans=True)
        ...
        self.ax3.text(
            1, 140, f"95% worst: ${trad_worst:.0f}", ha="center", fontsize=10, color="#ff6b6b"
        )
        self.ax3.text(
            2, 140, f"95% worst: ${nag_worst:.0f}", ha="center", fontsize=10, color="#00ff88"
        )
        ...
        self.ax3.text(
            0.5,
            0.9,
            f"⬆️ {improvement:.0f}% Robustness Improvement",
    ...
    def show_algorithm_comparison(self):
        ...
        self.ax4.text(0.5, 0.85, "Traditional", fontsize=11, fontweight="bold", color="#ff6b6b")
        self.ax4.text(0.8, 0.85, "NAG-QAOA", fontsize=11, fontweight="bold", color="#00ff88")
    ...
    def show_convergence_guarantee(self):
        ...
        self.ax5.semilogy(
            ...
            linewidth=2,
            alpha=0.7,
            label="Traditional (no guarantee)",
        )
        self.ax5.semilogy(
            iterations, nag_error, "-", color="#00ff88", linewidth=3, label="NAG-QAOA (proven)"
        )

        # Convergence threshold
        threshold = 0.5
        self.ax5.axhline(
            y=threshold, color="white", linestyle="--", alpha=0.5, label="Target accuracy"
        )
        ...
        self.ax5.text(
            0.5,
            0.15,
            "Theorem: P(|⟨H⟩_noisy - H_opt| < ε) > 1 - δ",
        ...
        self.ax5.text(
            0.5,
            0.05,
            "Convergence in O(log(1/ε)/γ²) iterations",
            transform=self.ax5.transAxes,
            ha="center",
            fontsize=10,
            color="white",
    ...
    def show_performance_metrics(self):
        ...
        trad_norm = [
            v / max(traditional_values[i], nag_values[i]) * 100
            for i, v in enumerate(traditional_values)
        ]
        nag_norm = [
            v / max(traditional_values[i], nag_values[i]) * 100 for i, v in enumerate(nag_values)
        ]
        ...
        self.ax6.text(
            0.5,
            0.9,
            f"🏆 {robustness:.0f}% Robustness Gain",

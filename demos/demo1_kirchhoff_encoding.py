...
class KirchhoffPreservingDemo:
    ...
    def setup_visualization(self):
        ...
        self.fig.suptitle(
            "QuantumGridOS Innovation #1: Kirchhoff-Preserving Quantum Encoding",
            fontsize=20,
            fontweight="bold",
            color="#00ff88",
        )
    ...
    def draw_network(self):
        ...
        node_colors = [
            "#ff6b6b" if self.bus_types[n] == "Generator" else "#4dabf7" for n in self.G.nodes()
        ]

        nx.draw_networkx_nodes(
            self.G, self.pos, node_color=node_colors, node_size=1500, ax=self.ax1
        )
        nx.draw_networkx_edges(self.G, self.pos, edge_color="#666666", width=3, ax=self.ax1)

        # Labels
        labels = {i: f"Bus {i}\n{self.bus_types[i]}" for i in self.G.nodes()}
        nx.draw_networkx_labels(
            self.G, self.pos, labels, font_size=10, font_color="white", ax=self.ax1
        )
        ...
        for bus in self.G.nodes():
            ...
            if self.generation[bus] > 0:
                self.ax1.text(
                    x,
                    y + 0.15,
                    f"Gen: {self.generation[bus]} MW",
                    ha="center",
                    fontsize=9,
                    color="#51cf66",
                )
            self.ax1.text(
                x, y - 0.15, f"Load: {self.loads[bus]} MW", ha="center", fontsize=9, color="#ff8787"
            )
    ...
    def show_power_flow(self):
        ...
        self.ax3.text(
            0.5,
            0.85,
            f"Balance: {total_gen - total_load} MW = 0 ✓",
            transform=self.ax3.transAxes,
            ha="center",
            fontsize=11,
            color="#00ff88",
    ...
    def show_traditional_encoding(self):
        ...
        self.ax4.text(
            0.5,
            0.2,
            f"Physics Violations: {violations}/{n_states}",
    ...
    def show_our_encoding(self):
        ...
        self.ax5.text(
            0.5,
            0.2,
            f"Physics Violations: 0/{n_states}",
        ...
        self.ax5.text(
            0.5,
            0.9,
            "✓ Guaranteed: ∑P_in = ∑P_out for ALL states",
            transform=self.ax5.transAxes,
            ha="center",
            fontsize=11,
            color="#00ff88",
    ...
    def draw_quantum_circuit(self):
        ...
        for pos, gate_type, color in zip(gate_positions, gate_types, gate_colors):
            for i in range(n_qubits):
                ...
                if gate_type == "KPL" and i < n_qubits - 1:
                    ...
                    # Single-qubit gate
                    circle = Circle((pos, y), 0.2, facecolor=color, edgecolor="white", linewidth=2)
        ...
        self.ax2.text(
            2,
            -0.8,
            "KPL = Kirchhoff Power Law Gate (Our Innovation)",
            ha="center",
            fontsize=10,
            color="#00ff88",
            style="italic",

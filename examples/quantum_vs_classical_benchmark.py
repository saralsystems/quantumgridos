...
class QuantumVsClassicalBenchmark:
    ...
    def benchmark_physics_preservation(self):
        ...
        for size in [14, 30, 57]:
            ...
            print(f"\n  ⚡ SPEEDUP: {speedup:.2f}×")
            print(
                f"  ✅ VALIDITY IMPROVEMENT: {quantum_result['valid_percent'] - classical_result['valid_percent']:.1f}%"
            )
    ...
    def benchmark_eigenvalue_analysis(self):
        ...
        for size in [14, 30, 57, 118]:
            ...
            print(f"\n  ⚡ SPEEDUP: {speedup:.2f}×")
            print(
                f"  📉 MEMORY REDUCTION: {(1 - quantum_result['qubits']*32/8/1e6 / classical_result['memory_mb'])*100:.1f}%"
            )
    ...
    def _quantum_noise_adaptive(self, network, uncertainty) -> Dict:
        ...
        # Noise profile
        noise_profile = {"T1": 150e-6, "T2": 100e-6, "gate_error": 0.001}
    ...
    def _is_valid_solution(self, solution, network) -> bool:
        ...
        return self._check_power_balance(solution, network) and self._check_voltage_limits(solution)
    ...
    def print_summary(self):
        ...
        for algo in algorithms:
            ...
            for size in [14, 30, 57, 118]:
                result = next(
                    (r for r in self.results if r.algorithm == algo and r.network_size == size),
                    None,
                )
                if result:
            ...
            print(
                f"{algo:<20} {speedups[0]:<12} {speedups[1]:<12} {speedups[2]:<12} {speedups[3]:<12}"
            )

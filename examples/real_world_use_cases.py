...
class MicrogridIntegrationUseCase:
    ...
    def _create_hospital_microgrid(self) -> PowerNetwork:
        ...
        network.add_generator(Generator(
            1, 5, "Diesel_Backup", pmin=0, pmax=2.0,  # 2MW diesel
            cost_b=150  # High cost
        ))
        network.add_generator(Generator(
            2, 4, "Solar_Roof", pmin=0, pmax=0.5,  # 500kW solar
            cost_b=0  # Zero marginal cost
        ))
    ...
    def _create_main_grid(self) -> PowerNetwork:
        ...
        network.add_generator(Generator(
            100, 100, "Grid", pmin=0, pmax=100, cost_b=50
        ))
        return network
...
class RenewablePlantStabilityUseCase:
    ...
    def _create_solar_farm(self) -> PowerNetwork:
        ...
        for i in range(1, 21):
            ...
            network.add_generator(Generator(
                i, i, f"Solar_{i}", pmin=0, pmax=5, cost_b=0
            ))
...
class ExtremeWeatherContingencyUseCase:
    ...
    def _create_coastal_grid(self) -> PowerNetwork:
        ...
        # Substations
        stations = [
            "Coastal_A", "Coastal_B", "Inland_A", "Inland_B",
            "Critical_Hospital", "Data_Center", "Water_Plant",
            "Emergency_Center", "Nuclear_Plant", "Load_Center"
        ]
    ...
    async def demonstrate_hurricane_contingency(self):
        ...
        n_lines = 15
        n_scenarios = int(np.math.factorial(n_lines) / 
                         (np.math.factorial(k) * np.math.factorial(n_lines - k)))
...
class HighRenewableOptimizationUseCase:
    ...
    def _create_california_simplified(self) -> PowerNetwork:
        ...
        # Major zones
        zones = [
            "NorCal", "BayArea", "Central", "LA_Basin", "SanDiego"
        ]

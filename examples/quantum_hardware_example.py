...
async def example_rigetti():
    ...
    backend = QuantumGridBackend(
        provider='rigetti',
        api_token=api_key,
        backend_name='Aspen-M-3'  # Latest Rigetti QPU
    )
        ...
        async def solve_unit_commitment(self, generators, demand):
            ...
            for i, (gen, commit) in enumerate(zip(generators[:n_gens], commitment)):
                if commit:
                    ...
                    output = min(gen['pmax'], demand * gen['pmax'] / sum(
                        g['pmax'] for g, c in zip(generators[:n_gens], commitment) if c
                    ))
                    total_output += output

...
from quantumgridos.power_systems.network import PowerNetwork, UnitCommitmentProblem
...
class PowerSystemSimulator:
    ...
    async def generate_stream(self) -> PowerSystemData:
        ...
        for line in self.network.lines.values():
            ...
            theta = (
                self.network.buses[line.from_bus].voltage_angle
...
async def example_maxcut_partitioning():
    ...
    # Setup QAOA solver
    qaoa_config = QAOAConfig(layers=3, optimizer="COBYLA", shots=1024)
...
async def example_real_time_streaming():
    ...
    interface = QuantumPowerInterface(
        quantum_backend="qiskit_aer", tcp_host="localhost", tcp_port=5000
    )
    ...
    for step in range(10):
        ...
        result = await interface.process_with_quantum(data, algorithm="qaoa", layers=2)
...
async def example_state_estimation():
    ...
    # Solve with VQE
    vqe = PowerSystemVQE(VQEConfig(ansatz="RealAmplitudes", optimizer="SLSQP", reps=2))
    ...
    print("  ---------|---------|-----------|-------")
    for i, (true_val, est_val) in enumerate(
        zip(true_state[: len(result["state_estimate"])], result["state_estimate"])
    ):

...
class MaxCutOptimizer:
    ...
    def __init__(self, network: PowerNetwork, algorithm: str = "qaoa", layers: int = 3, **kwargs):
    ...
    async def solve_async(self, data: Optional[Dict] = None) -> Dict:
        ...
        # Solve partitioning
        result = await asyncio.to_thread(self.solver.solve_maxcut, self.network.graph)
...
class UnitCommitment:
    ...
    def solve(self, period: Optional[int] = None) -> Dict:
        ...
        if period is not None:
            ...
            demand = (
                self.demand_forecast[period]
                if period < len(self.demand_forecast)
...
class OptimalPowerFlow:
    ...
    def solve(self) -> Dict:
        ...
        lines_dict = [{"from": l.from_bus, "to": l.to_bus, "capacity": l.rating} for l in lines]
        generators_dict = [
            {

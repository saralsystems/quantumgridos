...
class PowerSystemVQE:
    ...
    def create_opf_hamiltonian(self,
                              buses: List[Dict],
                              lines: List[Dict],
                              generators: List[Dict]) -> SparsePauliOp:
        ...
        for bus_idx, bus in enumerate(buses):
            ...
            gens_at_bus = [i for i, g in enumerate(generators) 
                          if g.get('bus', -1) == bus_idx]
    ...
    def create_state_estimation_hamiltonian(self,
                                           measurements: np.ndarray,
                                           measurement_matrix: np.ndarray) -> SparsePauliOp:
    ...
    def optimize(self,
                ...
                initial_point: Optional[np.ndarray] = None) -> Tuple[np.ndarray, float, Dict]:
    ...
    def solve_opf(self,
        ...
        for i, gen in enumerate(generators):
            ...
            dispatch.append({
                'generator': gen.get('name', f'Gen_{i}'),
                'output': output,
                'cost': cost
            })
            total_cost += cost
    ...
    def solve_state_estimation(self,
                              measurements: np.ndarray,
                              measurement_matrix: np.ndarray,
                              measurement_covariance: Optional[np.ndarray] = None) -> Dict:
        ...
        """
        # Create Hamiltonian
        hamiltonian = self.create_state_estimation_hamiltonian(
        ...
        for bitstring, count in counts.items():
            ...
            state_vector = np.array([
                1.0 if bit == '1' else 0.0 for bit in bitstring[:n_states]
            ])
    ...
    def adaptive_vqe(self,
        ...
        while current_reps <= 5:  # Max depth
            ...
            if results:
                ...
                initial_point[len(prev_params):] = np.random.uniform(-0.1, 0.1, 
                                                                     n_new_params - len(prev_params))
            else:
                initial_point = None
            
            # Optimize
            opt_params, energy, info = self.optimize(
                hamiltonian, ansatz, initial_point
            )
...
def solve_opf_quantum(buses: List[Dict],
                     lines: List[Dict],
...
def estimate_power_state(measurements: np.ndarray,
                        H_matrix: np.ndarray,
                        **kwargs) -> Dict:
